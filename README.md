...
The top-level `Exposition/` directory is a mixed publication surface. `Theory.md`, `Theorems.md`, `Identification.md`, and `Formalization.md` are hand-authored sources. `index.md` and `Glossary.md` are generated by `lake exe exposition_gen` and retain their `GENERATED ... do not edit` headers; `Assumptions.md` is rendered separately by `assumptions_gen` and written into the full exposition bundle.
