...
The architectural and definability fact under that sentence is the same one the Lean `Config` makes public: **nothing self-indexed is stored**. The field carries series, seeds, dispositions, delivery, and the re-pitched configuration the next deed reads, but the configuration has no owner-typed slot; agent relabelling leaves it fixed, commutes with re-pitching, and admits no equivariant recovery of an agent. This is not agent-blindness: delivery is a relation on tagged welds, `SameAgentDelivery` is deliberately field vocabulary, and a stored grade may even coincide extensionally with a tag in a particular model. It does not thereby carry a mineness or an index *as index*. What is indexed is welded at act-time, spent at act-time, and then available only as a field-describable occurrence-fact.
